  ...
  async (t) => {
    ...
    try {
      await t.test("from zero: opening migrated the empty database to the current schema", async () => {
        ...
        assert.equal(diag.schema_version, 4, "all migrations applied on first open");
        const tables = Object.keys((await store.overview(1)).tables).sort();
        assert.deepEqual(
          tables,
          ["activity", "edge", "item", "item_label", "meta", "raw", "source", "sync_run", "sync_state"],
  ...
  async () => {
    ...
    try {
      ...
      try {
        ...
        assert.equal(diag.driver, "postgres");
        assert.equal(diag.schema_version, 4, "the migrated store opens at the current schema");

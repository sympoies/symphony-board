...
export interface RepoMetricActorDTO {
  ...
  aliases?: string[];
  // Canonical provider profile page for this actor (added in 3.4.0). Usually
  // `https://<host>/<username>` on a supported GitHub/GitLab source; GitHub App
  // bots may use the provider-reported `https://<host>/apps/<slug>` URL. Emitted
  // for a `provider-user:<source_id>:<username>` identity, and for a
  // config-merged `person:<slug>` identity via the provider username observed on
  // this source (a `person` row is per-source). Omitted when no username was
  // observed here (email/name-keyed authorship) and for unsupported sources —
  // config-declared usernames are host-agnostic, so they are never guessed onto a
  // source. A display/navigation convenience, not identity.

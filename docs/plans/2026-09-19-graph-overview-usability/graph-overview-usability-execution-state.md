...
| --- | --- | --- | --- | --- |
| 1.1 | Capture Graph regressions | done | Baseline production-bundle assertion failed because the visible `not drawn` cue remained | Added multi-node partition/packing and rendered Force/Hierarchy/off-window checks |
| 1.2 | Implement component-aware Graph presentation | done | UI tests 450/450; UI build and render smoke pass | Preserved candidate/drawn ranking and focused layout; component links are assigned in one pass |
...
- 2026-09-20: a baseline production-build assertion captured the shipped `not
  drawn` cue before the fix. Implementation and review repairs completed with
  450/450 UI tests, a clean production UI build, and a passing render smoke.
...
- Specialist review strengthened rendered coverage for `off-window` and both
  layout modes, and removed the component partitioner's repeated edge scans.

...
- **Graph**: relationship view built from edge-connected items. It supports an
  ...
  re-entry during the app session. All edges, including mentions, are walked as
  undirected for membership while retaining their original direction. The
  projection is deterministic across cycles and caps a response
...
- **Commits**: commit-only SCM log over `activities[]`, styled separately from
  ...
  and resets detail/files scrolling. Newer/Older controls sit beneath the detail;
  in the phone reader they stay at the viewport bottom outside both scrolling
  panes. The entire reader accepts swipes, including blank space, just like Live.

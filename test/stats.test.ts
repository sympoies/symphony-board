...
test("buildStoreStats summarizes rows, breakdowns, tombstones, and sync runs", async () => {
  ...
  try {
    ...
    await db.finishRun(ok, "ok", "2026-06-01T00:00:30Z", 2, 1, 1, 4, 11, 0, null);
    const failed = await db.startRun(SOURCE, "incremental", "2026-06-01T00:02:00Z");
    await db.finishRun(failed, "error", "2026-06-01T00:02:05Z", 0, 0, 0, 1, 2, 1, "boom");
  } finally {
    await db.close();
  }
  ...
  assert.equal(stats.sync_runs[0]!.graphql_cost, 2);
  assert.equal(stats.sync_runs[0]!.graphql_cost_unknown, 1);
  ...
  assert.equal(stats.sync_runs[1]!.graphql_cost, 11);
  assert.equal(stats.sync_runs[1]!.graphql_cost_unknown, 0);

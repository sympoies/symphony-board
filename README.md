...
- `emit` produces contract major v4, currently `4.7.1`, and validates the JSON
...
- The read-only API sidecar also exposes operational views such as bounded
  canonical-history graph neighborhoods, store stats, review-cleanup
  candidates, and actionable open-work buckets for thin local automation.
  These endpoints read the canonical store; they are not the semver contract.
...
The current emitted contract is major v4, currently `4.7.1`. The canonical

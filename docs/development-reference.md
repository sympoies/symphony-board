...
Detailed development reference for `symphony-board`. The root
[`DEVELOPMENT.md`](../DEVELOPMENT.md) owns maintenance principles and the normal
change workflow. Read [`README.md`](../README.md) for the product overview,
[`running.md`](running.md) for operational run paths, [`DESIGN.md`](DESIGN.md)
for the design record, and [`CONTRACT.md`](CONTRACT.md) before changing the
emitted contract.
...
Follow [`CONTRACT.md`](CONTRACT.md):
...
Live webhook setup live in [`running.md`](running.md). Keep mandatory

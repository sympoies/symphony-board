...
export function startupRouteHash(restoredHash: string, defaultTab: Page): string {
  ...
  // Page-local sort toggles are deliberate view preferences, route-backed so a
  // shared link / reload keeps them. A bare ?itemSort=open / ?reviewSort=grouped
  // cold start must land on that page, not the default tab.
  if (route.page === "items" && route.itemSort) return restoredHash;

...
`person` identities through the provider username recovered during the identity
merge — the username observed on that source (a `person` row is per-source), or
the identity's single config-declared username when it was matched only by commit
name/email. Only authorship with no recoverable username (raw email/name) renders
unlinked.

-- symphony-board — cached per-repo all-time activity coverage bounds.
--
-- /api/range must keep activity rows range-bounded while repo coverage
-- (observed_since / last_activity_at) stays all-time. Cache those per-repo
-- bounds so the read path is O(repos), not O(activity history).

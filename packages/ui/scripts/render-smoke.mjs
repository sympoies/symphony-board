...
try {
  ...
  const checks = [
    ...
    [liveMarkdownSanitize.hasTable === true && liveMarkdownSanitize.keepsSafeRawTag === true && liveMarkdownSanitize.stripsScript === true && liveMarkdownSanitize.stripsImg === true && liveMarkdownSanitize.linkRendered === true && liveMarkdownSanitize.stripsJsHref === true && liveMarkdownSanitize.xssNotExecuted === true, `live: react-markdown renders GFM tables + raw HTML through the hardened sanitize schema — keeps <kbd>/<sub>, strips <script>/<img>, neutralizes javascript: hrefs, no XSS executed (${JSON.stringify(liveMarkdownSanitize)})`],
